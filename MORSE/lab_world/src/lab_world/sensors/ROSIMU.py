...
def GetIMUMsg(rates, accel):
    imu = Imu()
    imu.header.frame_id = 'world'
    imu.header.stamp = rospy.Time.from_sec(0)
    imu.orientation.x = 0
    imu.orientation.y = 0
    imu.orientation.z = 0
    imu.orientation.w = 0
    ...
    imu.angular_velocity.x = rates[0]
    imu.angular_velocity.y = rates[1]
    imu.angular_velocity.z = rates[2]
    return imu

class Rosimu(morse.core.sensor.Sensor):
    """
    THIS SENSOR IS AN ADAPTATION OF THE ORIGINAL MORSE IMU. IT HAS THE MAGNOMETER REMOVED
    ...
    def __init__(self, obj, parent=None):
        ...
        self.clkPub = rospy.Publisher('/clock', Clock, queue_size=10)
    ...
    def default_action(self):
        ...
        self.imuPub.publish(GetIMUMsg(rates, accel))
        self.clkPub.publish(rospy.Time.from_sec(time.time()-self.start_time))
